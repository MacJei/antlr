...
class Select(SqlServerStatements):
    ...
    def toPyspark(self, cnv_ds):        
        ...
        try:
            ...
            if WithTableGrp: 
                cnv_ds.cntx.logger.add_log('INFO','Preparing Dataframe for the WITH statement -')
                WithTable = WithTableGrp.group(1).strip()
                #get table df name
                if WithTable not in cnv_ds.table_df_map.keys():
                    cnv_ds.cntx.logger.add_log('WARN',f'{WithTable} not present in the Table-DF Dictionary. Creating DF for the {WithTable}')
                ...
                cnv_ds.cntx.logger.add_log('INFO','Preparing Dataframe for the SELECT INTO Clause -')
                #get table df name
            ...
            if SelectDf == 'select__df':
                if re.search(r'\bFROM\b', stmt, re.S|re.I):
                    ...
                    cnv_ds.cntx.logger.add_log('WARN','Following SELECT statement is not supported-')
                    cnv_ds.cntx.logger.add_log_details(self.original_text)
            
        except Exception as e:
            raise
            self.cntx.logger.add_log('ERROR','Error occurred while converting the following SELECT statement -')
...
class Insert(SqlServerStatements):
    ...
    def toPyspark(self, cnv_ds):
        ...
        if ins_sel:    
            try:
                cnv_ds.cntx.logger.add_log('INFO','INSERT Statement with SELECT conversion begins')
                ...
                if SrcSelect:
                        ...
                        cnv_ds.cntx.logger.add_log('INFO','INSERT Statement with SELECT conversion completed.')
            except Exception as e:
                cnv_code = ''
                cnv_ds.cntx.logger.add_log('ERROR','Error occurred while converting the following INSERT statement -')
                cnv_ds.cntx.logger.add_log_details(stmt)
                raise                        

        if ins_val:
            try:
                cnv_ds.cntx.logger.add_log('INFO','INSERT Statement with COLUMN Values conversion begins.')
                cnv_ds.cntx.logger.add_log_details(stmt)
                ...
                cnv_ds.cntx.logger.add_log('INFO','INSERT Statement with COLUMN Values conversion completed.')
