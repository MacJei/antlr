...
class SqlServerStatements():
    ...
    def scopeCheckChange(self, cnv_ds):
        ...
        if re.search(r'\+', stmt, re.S|re.I):
            ...
            if not(re.search(r'^SET\s+@\w+\s*=', stmt, re.M|re.I)):
                cnv_ds.cntx.logger.add_log('INFO', "Checking for hard coded string and converting to string contination operator '||'")
                stmt = re.sub(r'(\'\s*)\+', '\\1||', stmt, flags=re.S)
                stmt = re.sub(r'\+(\s*\')', '||\\1', stmt, flags=re.S)
            
        #check for WITH LOCK
        re_match = re.search(r'WITH?\s*\(\s*(NOLOCK|TABLOCK).*?\)', stmt, re.S|re.I)
        if re_match:
            str = re_match.group()
    ...
    def toPyspark(self, cnv_ds):        
        ...
        try:
            ...
            if with_table_grp:
                ...
                cnv_ds.with_tables.append(with_table)
