...
class SqlServerStatements():
    ...
    def replaceOperator(self, cnv_ds, cond_str):
        ...
        cnv_ds.cntx.logger.add_log('INFO', 'Modified below conditional expression from')
    ...
    def toPyspark(self, cnv_ds):
        try:    
            ...
            try:
                ...
                cnv_ds.cntx.logger.add_log_details(stmt)
                cnv_ds.cntx.logger.add_log_details('Syntax error: ' + str(e))
                cnv_ds.cntx.logger.add_log('WARN', 'Using unparsed statement. Result may be inconsistent.')                
            else:    
                stmt = lsnr.out_sql.replace('__LOAD__', 'load').replace('__BLOCK__', 'block').replace('__PLATFORM__', 'platform') + ';'     
                cnv_ds.cntx.logger.add_log('INFO', 'Statement parse completed.')
            cnv_ds.cntx.logger.add_log("INFO","Merge Statement Conversion Started")
            ...
            if re.search(r'\bUSING\s*\(\s*SELECT', stmt, re.S|re.I):
                ...
                cnv_ds.cntx.logger.add_log("INFO","Merge from table is in the below subquery")
            ...
            insert_str_match = re.search(r'\bINSERT?\s+(.*?)(?:(?:WHEN\b)|;)', stmt, re.S|re.I)           
            if insert_str_match:
            ...
            cnv_ds.cntx.logger.add_log("INFO","Merge Statement Conversion Completed")
            cnv_ds.cntx.logger.add_log("INFO","My conversion begins")
            #upd_obj=Update(stmt)
            #ins_obj=Insert(stmt)
            if re.search(r'\bINSERT?\s+(.*?)(?:(?:WHEN\b)|;)', stmt, re.S|re.I):
                insert_str_match = re.search(r'\bINSERT?\s+(.*?)(?:(?:WHEN\b)|;)', stmt, re.S|re.I)
                insert_str = insert_str_match.group().strip()
                ins_obj=Insert(insert_str)
                c=ins_obj.toPyspark(cnv_ds)
                #print(c)

            #find_upd=re.search(r'',stmt, re.S|re.I).group(1).strip()
            #find_ins=re.search(r'',stmt, re.S|re.I).group(1).strip()
            cnv_ds.cntx.logger.add_log("INFO","My conversion ends")
          
        except Exception as e:
            cnv_ds.cntx.logger.add_log("ERROR","Exception : Error occurred at line {} while converting the following MERGE statement".format(sys.exc_info()[-1].tb_lineno))
            cnv_ds.cntx.logger.add_log_details(self.original_text)
            cnv_ds.cntx.logger.add_log_details(str(e))
            cnv_code = ''            
