...
class SqlServerConvert():
    ...
    def scanScript(self):
        ...
        self.cntx.logger.add_log('INFO', 'Scanning script context for all the tables used')
        ...
        if self.cntx.out_code_type == 'pyspark':
            ...
            if self.cntx.platform == 'glue':
                ...
                if len(self.cnv_ds.table_df_map) > 0:
                    ...
                    for tab, df in self.cnv_ds.table_df_map.items():
                        if (tab[0] != '#') and ('.#' not in tab.replace(' ','')) and ('.[#' not in tab.replace(' ','')):
                            dot_count = tab.count('.')
                            if dot_count == 0:
                                tab = 'dbo.' + tab
                            if dot_count > 1: 
                                tab = '.'.join(tab.split('.')[-2:])
                            tab = tab.replace('[', '').replace(']', '')
                            code_txt += f"'{tab}':'{df}',\n" + ' '*4*2
                    code_txt = code_txt.rstrip().strip(',') + '}\n\n'
    ...
    def addPlatformWrite(self):
        ...
        if self.cntx.out_code_type == 'pyspark':
            if self.cntx.platform == 'glue':
                ...
                #pyspark code to write dataframes to target database
